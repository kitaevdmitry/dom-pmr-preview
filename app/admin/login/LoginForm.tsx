...
export default function LoginForm(){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{const {error}=await createClient().auth.signInWithPassword({email,password});if(error){setStatus("Неверный email или пароль");return}location.href="/admin"}catch{setStatus("Не удалось войти. Попробуйте ещё раз.")}finally{setLoading(false)}
  }
