...
export default function PropertyActions({id,title}:{id:number;title:string}){
  ...
  return <div className="property-tools"><button className={saved?"saved":""} onClick={toggle} aria-pressed={saved} data-track={saved?undefined:"favorite_add"} data-property-id={id}>♡ {saved?"В избранном":"В избранное"}</button><button onClick={share} data-track="share" data-property-id={id}>↗ Поделиться</button><span aria-live="polite">{message}</span></div>;
}
