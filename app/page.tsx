...
function SmartImage({src,alt,eager=false}:{src:string;alt:string;eager?:boolean}){const [loaded,setLoaded]=useState(false),[fallback,setFallback]=useState(false);return <span className={`image-shell ${loaded?"loaded":""}`}>
...
function CatalogGallery({property,eager,onOpen}:{property:Property;eager:boolean;onOpen:()=>void}){
  const images=Array.from(new Set([property.image,...(property.gallery??[])].filter(Boolean))).slice(0,10),track=useRef<HTMLDivElement>(null),[active,setActive]=useState(0);
  ...
  return <div className="catalog-gallery" aria-label={`Фотографии: ${property.title}`} tabIndex={0} onKeyDown={event=>{if(event.key==="ArrowLeft")move(-1);if(event.key==="ArrowRight")move(1)}}>
    <div className="catalog-gallery-track" ref={track} onScroll={sync}>{images.map((src,index)=><Link href={`/object/${property.id}`} onClick={onOpen} aria-label={`Открыть объект, фотография ${index+1} из ${images.length}`} key={`${src}-${index}`}><SmartImage src={src} alt={`${property.title}, фотография ${index+1}`} eager={eager&&index===0}/></Link>)}</div>
...
function SellerForm(){
  ...
  async function submit(event:React.FormEvent<HTMLFormElement>){event.preventDefault();setStatus("Отправляем…");const form=event.currentTarget;const response=await fetch("/api/leads",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(Object.fromEntries(new FormData(form)))});if(response.ok){form.reset();setStatus("Спасибо! Мы свяжемся с вами для бесплатной консультации.")}else setStatus("Не удалось отправить. Позвоните нам по номеру 0 (777) 88-308.")}
  return <form className="seller-form" onSubmit={submit}><div className="seller-fields"><label>Как к вам обращаться?<input name="name" required placeholder="Ваше имя"/></label><label>Телефон<input name="phone" required type="tel" placeholder="0 (___) ___-___"/></label><label>Город<select name="city"><option>Тирасполь</option><option>Бендеры</option><option>Днестровск</option><option>Слободзея</option><option>Рыбница</option><option>Дубоссары</option><option>Другой</option></select></label><label>Что продаём?<select name="propertyType"><option>Квартира</option><option>Дом</option><option>Участок</option><option>Коммерция</option></select></label></div><label className="seller-note">Коротко об объекте<textarea name="note" placeholder="Район, площадь, состояние — если удобно"/></label><button type="submit">Получить консультацию по продаже →</button><small>Свяжемся только по вопросу вашего объекта. Без навязчивых рассылок.</small>{status&&<p role="status">{status}</p>}</form>
}
...
export default function Home(){
  ...
  const [selected,setSelected]=useState<Property|null>(null),[saved,setSaved]=useState<number[]>([]),[favoritesReady,setFavoritesReady]=useState(false),[favoritesOnly,setFavoritesOnly]=useState(false),[filtersReady,setFiltersReady]=useState(false),[filtersOpen,setFiltersOpen]=useState(false),[mobileMode,setMobileMode]=useState<"map"|"list">("list"),[mapSeen,setMapSeen]=useState(false),[isMobile,setIsMobile]=useState(true);
  useEffect(()=>{const media=matchMedia("(max-width:700px)"),sync=()=>setIsMobile(media.matches);sync();media.addEventListener("change",sync);return()=>media.removeEventListener("change",sync)},[]);
  useEffect(()=>{if(!filtersOpen)return;const previous=document.body.style.overflow;document.body.style.overflow="hidden";return()=>{document.body.style.overflow=previous}},[filtersOpen]);
  ...
  const showMap=!isMobile||mapSeen;
  const openMap=()=>{setMapSeen(true);setMobileMode("map")};
  const rememberPosition=()=>sessionStorage.setItem("dinastiya-catalog-scroll",String(scrollY));
...
<a className="header-phone" href="tel:+37377788308">777 88 308</a>
<a className="outline-button" href="#contacts">Консультация</a>
</div>
</header>
...
<button onClick={()=>setFiltersOpen(true)}>☰ Фильтры {activeFilters>0&&<b>{activeFilters}</b>}</button>
<div>
<button className={mobileMode==="list"?"active":""} onClick={()=>setMobileMode("list")}>☷ Список</button>
<button className={mobileMode==="map"?"active":""} onClick={openMap}>⌖ Карта</button>
</div>
</div>
      <div className={`filters ${filtersOpen?"open":""}`}>
<div className="filter-mobile-head">
<strong>Что вы ищете?</strong>
<button onClick={()=>setFiltersOpen(false)} aria-label="Закрыть">×</button>
</div>
<label>
<span>Сделка</span>
<select value={deal} onChange={e=>changeDeal(e.target.value as "Любая сделка"|Deal)}>
...
<label>
<span>Город</span>
<select value={place} onChange={e=>{setPlace(e.target.value);setDistrict("Любой район")}}>{places.map(p=>
<option key={p}>{p}</option>)}</select>
</label>
<label>
<span>Район</span>
<select value={district} onChange={e=>setDistrict(e.target.value)}>{districts.map(p=>
<option key={p}>{p}</option>)}</select>
</label>
...
{activeFilters>0&&<div className="active-filter-chips" aria-label="Активные фильтры">{place!=="Все города"&&<button onClick={()=>{setPlace("Все города");setDistrict("Любой район")}}>{place} ×</button>}{district!=="Любой район"&&<button onClick={()=>setDistrict("Любой район")}>{district} ×</button>}{deal!=="Любая сделка"&&<button onClick={()=>setDeal("Любая сделка")}>{deal} ×</button>}{type!=="Любой тип"&&<button onClick={()=>setType("Любой тип")}>{type} ×</button>}{minPrice!==null&&<button onClick={()=>setMinPrice(null)}>от {new Intl.NumberFormat("ru-RU").format(minPrice)} $ ×</button>}{maxPrice!==null&&<button onClick={()=>setMaxPrice(null)}>до {new Intl.NumberFormat("ru-RU").format(maxPrice)} $ ×</button>}{minArea>0&&<button onClick={()=>setMinArea(0)}>от {minArea} м² ×</button>}{rooms>0&&<button onClick={()=>setRooms(0)}>{rooms===4?"4+":rooms} комн. ×</button>}{floorMode!=="Любой этаж"&&<button onClick={()=>setFloorMode("Любой этаж")}>{floorMode} ×</button>}{condition!=="Любое состояние"&&<button onClick={()=>setCondition("Любое состояние")}>{condition} ×</button>}{heating!=="Любое отопление"&&<button onClick={()=>setHeating("Любое отопление")}>{heating} ×</button>}{photosOnly&&<button onClick={()=>setPhotosOnly(false)}>С фото ×</button>}<button className="clear-all" onClick={reset}>Очистить всё</button></div>}
      <div className={`catalog-grid mode-${mobileMode}`}>
...
<button className={saved.includes(p.id)?"saved":""} onClick={e=>{e.stopPropagation();setSaved(current=>current.includes(p.id)?current.filter(id=>id!==p.id):[...current,p.id])}} aria-label={saved.includes(p.id)?"Убрать из избранного":"Сохранить объект"} aria-pressed={saved.includes(p.id)}>♡</button>
</div>
...
<aside className="map-panel">{showMap?<PropertyMap properties={filtered} selectedId={selected?.id} onSelect={pickProperty}/>:<div className="map-placeholder">Карта загрузится после выбора режима</div>}<div className="map-tip">Нажмите на цену, чтобы увидеть объект</div>{selected&&<div className="map-property">
...
<div className="team-intro"><p className="section-kicker">Личное сопровождение</p><h2 id="team-title">За объектами стоят<br/>реальные люди</h2><p>Не безликий колл-центр: вы общаетесь со специалистом, который знает рынок ПМР и ведёт вопрос до результата.</p><a href="https://www.instagram.com/dinastya_nedvijimost" target="_blank" rel="noreferrer">Познакомиться с командой в Instagram ↗</a></div>
<article className="specialist-card"><div className="specialist-portrait" aria-hidden="true"><span>ГР</span></div><div><small>Специалист по недвижимости</small><h3>Галина Рязанцева</h3><p>Консультация, подбор объекта и сопровождение сделки в Тирасполе и по ПМР.</p><div><a href="tel:+37377788308">Позвонить</a><a href="https://wa.me/37377788308" target="_blank" rel="noreferrer">WhatsApp</a></div></div></article>
<div className="instagram-proof"><b>6 500+</b><span>подписчиков следят за новыми объектами и работой команды</span><a href="https://www.instagram.com/dinastya_nedvijimost" target="_blank" rel="noreferrer">@dinastya_nedvijimost</a></div>
</section>
